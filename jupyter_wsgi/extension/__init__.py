...
def timeout(time):
    ...
    try:
        ...
        signal.alarm(time) # Schedule the signal to be sent after ``time``.
        unreg = True
    except AttributeError:
        # this will happen on Windows which does not have signal.alarm()
        unreg = False
    try:
        yield
    except TimeoutError:
...
def load_jupyter_server_extension(nb_server_app):
    ...
    for p in SYSTEM_CONFIG_PATH:
        try:
            ...
            IndexHandler.set_bad_yaml(True)
            app_log.warning(f'Failed to load {yamlfn}')
        else:
            IndexHandler.set_name( jupyter_wsgi_conf.get('name','Webapps') )
    ...
    if extension_path != 'wsgi':
        handlers.append( (url_path_join(jupyter_base_url,'wsgi'), IndexHandler)  )
